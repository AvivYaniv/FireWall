...
int setPacketVerdictByRulesTable(fw_packet_info* pPacketInfo)
{
	/* Variable Section */
	verdict_t			vVerdict;
...
void setPacketVerdictByRules(fw_packet_info* 	pPacketInfo)
{
	...
	#ifdef PRINT_DEBUG_MESSAGES
        printk(KERN_INFO SETTING_PACKET_VERDICT_BY_RULES_TABLE);
    #endif

	/* If no matching rule found */
	if (0 > setPacketVerdictByRulesTable(pPacketInfo))
	{
		pPacketInfo->action = NF_ACCEPT;
